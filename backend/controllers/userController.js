...
exports.authUser = asyncHandler(async (req, res) => {
    const { email, batchCode, password } = req.body

    const user = await User.findOne({ email: email })

    const batch = await User.findOne({ batchCode: batchCode })

    if (user && batch) {
        if (user && (await user.matchPassword(password))) {
            res.json({ _id: user._id, name: user.name, email: user.email, batchCode: user.batchCode, isAdmin: user.isAdmin, token: generateToken(user._id) })
        } else {
            res.status(404)
        ...
        res.status(404)
        throw new Error('Invalid Email or Batch code')
    }
...
exports.registerUser = asyncHandler(async (req, res) => {
    const { name, email, password, mobile, batchCode } = req.body
    ...
    const user = await User.create({
        name, email, mobile, password, batchCode
    })

    if (user) {
        res.status(201).json({ _id: user._id, name: user.name, mobile: user.mobile, email: user.email, batchCode: user.batchCode, isAdmin: user.isAdmin })
    } else {
        res.status(400)
